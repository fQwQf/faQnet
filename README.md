...
该项目采用了2024年诺贝尔物理学奖的思想方法，充分体现了其前沿性和前瞻性。  
...
- 构造函数  
	...
	传入参数：
	- 每一层节点详情  
	这是一个vector，储存一些vector，其中第二层每个vector代表一个节点详情，由节点数量和激活函数类型构成，如{{4，"sigmoid"}，{5，"tanh"}，{6，"relu"}}代表：第一层有4个节点且使用sigmoid激活函数，第二层有5个节点且使用tanh激活函数，第三层有6个节点且使用relu激活函数。
...
- 初始化矩阵  
	...
	下面是三种初始化方法简介：  
	- 均匀分布初始化  
	传入均匀分布的最小值和最大值，将矩阵中的每一项赋值为一个均匀分布的随机值。
	- 正态分布初始化  
	传入正态分布的均值和标准差，将矩阵中的每一项赋值为一个正态分布的随机值。
	- 常数初始化  
	传入一个常数，将矩阵中的每一项赋值为该常数。  
