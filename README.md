...
- 反向传播  
	这个函数接受前向传播的输出矩阵和目标矩阵，首先通过输出矩阵减去目标矩阵得到最后一层的误差矩阵，然后通过调用每一层的反向传播函数，将误差矩阵传入倒数第二层，将倒数第二层的误差矩阵传入倒数第三层，以此类推，直到第一层。  
...
- 输出数据反归一化  
	这个函数接受若干输出矩阵，然后将其反归一化。  
	传入参数：
	- 输出矩阵  

- 输出数据归一化  
	这个函数接受若干输出矩阵，然后将其归一化。  
	传入参数：
	- 输出矩阵  
