...
- 反向传播  
	这个函数接受前向传播的输出矩阵和目标矩阵，首先通过执行损失函数的导函数得到最后一层的输出对于损失值的偏导，然后代入最后一层的损失函数的导函数得到最后一层的误差矩阵（即线性运算的结果对于最终输出的损失值的偏导），将误差矩阵传入倒数第二层，将倒数第二层的误差矩阵传入倒数第三层，以此类推，直到第一层。  
...
- 单层反向传播  
	这个函数接受上一层的误差矩阵，首先将其与上一层权值矩阵的转置矩阵相乘得到一个中间矩阵，然后将其与这一层的激活函数在结果矩阵处的导函数值进行点乘即得到该层的误差矩阵，将其输出并保存。  
...
- 单层权值更新  
	这个函数接受一个学习率和上层误差矩阵。权值更新的具体操作是：将权值矩阵减去（学习率\*该层输入矩阵的转置矩阵\*该层误差矩阵）。注：只需将结果矩阵代入激活函数即可得到输出矩阵。  
...
- 单层偏置更新
	这个函数接受一个学习率和上层误差矩阵。偏置项更新的具体操作是：将偏置项矩阵减去（学习率\*该层误差矩阵）。  
